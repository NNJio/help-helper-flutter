...
class _OtpPageState extends ConsumerState<OtpPage> {
  ...
  void initState() {
    ...
    final otpMap = ref.read(mockOtpMapProvider);
    final mockOtp = otpMap[phone];
    //! Mock OTP
  ...
  Future<void> _onOtpFilled(String code) async {
    ...
    if (verificationId == null) {
      ...
      ref.read(otpLoadingProvider.notifier).state = false; // ❌ หยุดโหลด
      return;
    }
    ...
    try {
      ...
      ref.read(otpLoadingProvider.notifier).state = false; // ✅ หยุดโหลดก่อนเปลี่ยนหน้า
      if (!mounted) return;
      ...
      ref.read(otpLoadingProvider.notifier).state = false; // ❌ หยุดโหลดตอน error
    }
  }
